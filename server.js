...
function extractFocusPoints(stockInfo, tech, currentPrice, high52w = null, low52w = null) {
  ...
  if (s50 !== null) {
    if (currentPrice && currentPrice > s50)
      points.push({ title: 'SMA50 サポートライン', level: s50, importance: 5, description: `SMA50（¥${Math.round(s50).toLocaleString()}）は中期トレンドの支持線として機能する可能性。`, action: `¥${Math.round(s50).toLocaleString()} を割り込んだ場合、下降トレンド転換の可能性があるため要注意。` });
    else
  ...
  if (high52w !== null && high52w > currentPrice)
    ...
    points.push({ title: '52週高値（近接・更新中）', level: Math.round(high52w), importance: 5, description: `現在値が52週高値（¥${Math.round(high52w).toLocaleString()}）付近または更新中。`, action: `高値更新を維持できれば上昇継続。失速した場合は反落に警戒。` });
